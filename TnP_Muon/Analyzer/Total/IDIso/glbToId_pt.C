...
#include <TLine.h>
#define Nbin 4
void glbToId_pt(const int charge)
{
  if (charge==1)
  char  ch[20] = "plus"; 
  else if (charge==-1)
  ...
  if (charge==1){
    TFile *f_MC= new TFile("TnP_GlbToID_MCptplus_WptTight2012_pt.root","read");
    TFile *f_RD= new TFile("TnP_GlbToID_RDptplus_WptTight2012_pt.root","read");
  }else if (charge==-1){
    TFile *f_MC= new TFile("TnP_GlbToID_MCptminus_WptTight2012_pt.root","read");
    TFile *f_RD= new TFile("TnP_GlbToID_RDptminus_WptTight2012_pt.root","read");
  }
...
//  grMC->GetYaxis()->CenterTitle();
  ...
  RooDataSet *datasetRD = (RooDataSet*)f_RD->Get("tpTree/WptTight2012_pt/fit_eff");
