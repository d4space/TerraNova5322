...
void trig_pt(const int charge)
{
  ...
  if (charge==1){
    TFile *f_MC= new TFile("TnP_WptCutToTrig_MCptplus.root","read");
    TFile *f_RD= new TFile("TnP_WptCutToTrig_RDptplus.root","read");
  }else if (charge==-1){
    TFile *f_MC= new TFile("TnP_WptCutToTrig_MCptminus.root","read");
    TFile *f_RD= new TFile("TnP_WptCutToTrig_RDptminus.root","read");
  }
  ...
  RooDataSet *datasetRD = (RooDataSet*)f_RD->Get("tpTree/Tnp_WptCut_to_Mu15_eta2p1_pt/cnt_eff");
