...
#include <TLine.h>
#define Nbin 6
void trig_eta(const int filetype, const int charge)
{
  if (charge==1)
  char  ch[20] = "plus"; 
  else if (charge==-1)
  ...
  if (filetype==20){
    if (charge==1){
      TFile *f_MC= new TFile("TnP_WptCutToTrig_MCetaplus_20.root","read");
      TFile *f_RD= new TFile("TnP_WptCutToTrig_RDetaplus_20.root","read");
    }else if (charge==-1){
      TFile *f_MC= new TFile("TnP_WptCutToTrig_MCetaminus_20.root","read");
      TFile *f_RD= new TFile("TnP_WptCutToTrig_RDetaminus_20.root","read");
    }
  }else if (filetype==40){
    if (charge==1){
      TFile *f_MC= new TFile("TnP_WptCutToTrig_MCetaplus_40.root","read");
      TFile *f_RD= new TFile("TnP_WptCutToTrig_RDetaplus_40.root","read");
    }else if (charge==-1){
      TFile *f_MC= new TFile("TnP_WptCutToTrig_MCetaminus_40.root","read");
      TFile *f_RD= new TFile("TnP_WptCutToTrig_RDetaminus_40.root","read");
    }
  }else if (filetype==55){
    if (charge==1){
      TFile *f_MC= new TFile("TnP_WptCutToTrig_MCetaplus_55.root","read");
      TFile *f_RD= new TFile("TnP_WptCutToTrig_RDetaplus_55.root","read");
    }else if (charge==-1){
      TFile *f_MC= new TFile("TnP_WptCutToTrig_MCetaminus_55.root","read");
      TFile *f_RD= new TFile("TnP_WptCutToTrig_RDetaminus_55.root","read");
    }
  }else if (filetype==1000){
    if (charge==1){
      TFile *f_MC= new TFile("TnP_WptCutToTrig_MCetaplus.root","read");
      TFile *f_RD= new TFile("TnP_WptCutToTrig_RDetaplus.root","read");
    }else if (charge==-1){
      TFile *f_MC= new TFile("TnP_WptCutToTrig_MCetaminus.root","read");
      TFile *f_RD= new TFile("TnP_WptCutToTrig_RDetaminus.root","read");
    }
  }
  ...
  grMC->GetYaxis()->SetTitle("Trigger Efficiency");
  grMC->Draw("AP");

  RooDataSet *datasetRD = (RooDataSet*)f_RD->Get("tpTree/Tnp_WptCut_to_Mu15_eta2p1_eta/cnt_eff");
