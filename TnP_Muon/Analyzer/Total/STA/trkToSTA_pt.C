...
#include <TLine.h>
#define Nbin 4
void trkToSTA_pt(const int charge)
{
  if (charge==1)
  char  ch[20] = "plus"; 
  else if (charge==-1)
  ...
  if (charge==1){
    ...
    TFile *f_RD= new TFile("TnP_TrkToSTA_RDptplus_Glb_pt.root","read");
  }else if (charge==-1){
    TFile *f_MC= new TFile("TnP_TrkToSTA_MCptminus_Glb_pt.root","read");
    TFile *f_RD= new TFile("TnP_TrkToSTA_RDptminus_Glb_pt.root","read");
  }
  ...
  RooDataSet *datasetMC = (RooDataSet*)f_MC->Get("tpTree/Glb_pt/fit_eff");
  ...
  RooDataSet *datasetRD = (RooDataSet*)f_RD->Get("tpTree/Glb_pt/fit_eff");
