...
#include <TLine.h>
#define Nbin 6
void trkToSTA_eta(const int filetype, const int charge)
{
  if (charge==1)
  char  ch[20] = "plus"; 
  else if (charge==-1)
  ...
  if (filetype==20){
    if (charge==1){
      TFile *f_MC= new TFile("TnP_TrkToSTA_MCetaplus_20_Glb_eta.root","read");
      TFile *f_RD= new TFile("TnP_TrkToSTA_RDetaplus_20_Glb_eta.root","read");
    }else if (charge==-1){
      TFile *f_MC= new TFile("TnP_TrkToSTA_MCetaminus_20_Glb_eta.root","read");
      TFile *f_RD= new TFile("TnP_TrkToSTA_RDetaminus_20_Glb_eta.root","read");
    }
  }else if (filetype==40){
    if (charge==1){
      TFile *f_MC= new TFile("TnP_TrkToSTA_MCetaplus_40_Glb_eta.root","read");
      TFile *f_RD= new TFile("TnP_TrkToSTA_RDetaplus_40_Glb_eta.root","read");
    }else if (charge==-1){
      TFile *f_MC= new TFile("TnP_TrkToSTA_MCetaminus_40_Glb_eta.root","read");
      TFile *f_RD= new TFile("TnP_TrkToSTA_RDetaminus_40_Glb_eta.root","read");
    }
  }else if (filetype==55){
    if (charge==1){
      TFile *f_MC= new TFile("TnP_TrkToSTA_MCetaplus_55_Glb_eta.root","read");
      TFile *f_RD= new TFile("TnP_TrkToSTA_RDetaplus_55_Glb_eta.root","read");
    }else if (charge==-1){
      TFile *f_MC= new TFile("TnP_TrkToSTA_MCetaminus_55_Glb_eta.root","read");
      TFile *f_RD= new TFile("TnP_TrkToSTA_RDetaminus_55_Glb_eta.root","read");
    }
  }else if (filetype==1000){
    if (charge==1){
      TFile *f_MC= new TFile("TnP_TrkToSTA_MCetaplus_Glb_eta.root","read");
      TFile *f_RD= new TFile("TnP_TrkToSTA_RDetaplus_Glb_eta.root","read");
    }else if (charge==-1){
      TFile *f_MC= new TFile("TnP_TrkToSTA_MCetaminus_Glb_eta.root","read");
      TFile *f_RD= new TFile("TnP_TrkToSTA_RDetaminus_Glb_eta.root","read");
    }
  }
  ...
  RooDataSet *datasetMC = (RooDataSet*)f_MC->Get("tpTree/Glb_eta/fit_eff");
  ...
  grMC->GetYaxis()->SetTitle("STA Muon Efficiency");
  grMC->Draw("AP");
  
  RooDataSet *datasetRD = (RooDataSet*)f_RD->Get("tpTree/Glb_eta/fit_eff");
