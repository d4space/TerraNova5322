...
MuonIDVariables = cms.PSet(
    ...
    # For Wpt
    isTrakerMu = cms.string("? isTrackerMuon ? 1 : -1"),
    isPFMu = cms.string("? isPFMuon ? 1 : -1"),
    isGlbMu = cms.string("? isGlobalMuon ? 1 : -1"),
)
...
TrackQualityVariables = cms.PSet(
    ...
    tkDz = cms.string("? track.isNull ? -999: track.dz"),
    # Hits and such
...
MuonIDFlags = cms.PSet(
    ...
    WptTight2012   = cms.string("abs(eta) < 2.1 && isPFMuon && muonID('GlobalMuonPromptTight')"
      			+" && globalTrack.normalizedChi2() < 10 "
                         +" && globalTrack.hitPattern.numberOfValidMuonHits > 0 && numberOfMatchedStations > 1 "
			 #+" && muonID('GlobalMuonPromptTight')"
			 #+" && abs(dB) < 0.02 && abs(track.dz) < 5 " # cm, mm   /// we had a Muon effi problem. then we deleted dz cut from effi study.
			 +" && abs(dB) < 0.02 " # cm
                        +" && track.hitPattern.trackerLayersWithMeasurement > 5 && track.hitPattern.numberOfValidPixelHits > 0 "
    ...
    POgTight12   = cms.string("abs(eta) < 2.1 && isPFMuon && isGlobalMuon"
      			+" && globalTrack.normalizedChi2() < 10 "
                         +" && globalTrack.hitPattern.numberOfValidMuonHits > 0 && numberOfMatchedStations > 1 "
			 #+" && muonID('GlobalMuonPromptTight')"
			 #+" && abs(dB) < 0.02 && abs(track.dz) < 5 " # cm, mm   /// we had a Muon effi problem. then we deleted dz cut from effi study.
    ...
    Wpteta   = cms.string("abs(eta) < 2.1"),
    WptetaGlb   = cms.string("abs(eta) < 2.1 && isPFMuon && isGlobalMuon"),
    WptnormChi2   = cms.string("abs(eta) < 2.1 && isPFMuon && isGlobalMuon"
                          +" && globalTrack.normalizedChi2() < 10 "
                          ),
    WptNumValidMuon   = cms.string("abs(eta) < 2.1 && isPFMuon && isGlobalMuon"
                          +" && globalTrack.normalizedChi2() < 10 "
                          +" && globalTrack.hitPattern.numberOfValidMuonHits > 0 && numberOfMatchedStations > 1 "
                          ),
    WptdB = cms.string("abs(eta) < 2.1 && isPFMuon && isGlobalMuon"
                          +" && globalTrack.normalizedChi2() < 10 "
                           +" && globalTrack.hitPattern.numberOfValidMuonHits > 0 && numberOfMatchedStations > 1 "
		           #+" && muonID('GlobalMuonPromptTight')"
   	                   +" && abs(dB) < 0.02" # cm, mm
        	           ),
    WptdBdZ   = cms.string("abs(eta) < 2.1 && isPFMuon && isGlobalMuon"
      			+" && globalTrack.normalizedChi2() < 10 "
			+" && globalTrack.hitPattern.numberOfValidMuonHits > 0 && numberOfMatchedStations > 1 "
			#+" && muonID('GlobalMuonPromptTight')"
			#+" && abs(dB) < 0.02 && abs(track.dz) < 5 " # cm, mm  /// we had a Muon effi problem. then we deleted dz cut from effi study.
    ...
    WptWoIso   = cms.string("abs(eta) < 2.1 && isPFMuon && isGlobalMuon"
      			+" && globalTrack.normalizedChi2() < 10 "
                         +" && globalTrack.hitPattern.numberOfValidMuonHits > 0 && numberOfMatchedStations > 1 "
			 #+" && muonID('GlobalMuonPromptTight')"
			 #+" && abs(dB) < 0.02 && abs(track.dz) < 5 " # cm, mm /// we had a Muon effi problem. then we deleted dz cut from effi study.
...
HighPtTriggerFlags = cms.PSet(
   ...
   DoubleMu17Mu8_Mu17 = cms.string("!triggerObjectMatchesByFilter('hltL3fL1DoubleMu10MuOpenL1f0L2f10L3Filtered17').empty() || !triggerObjectMatchesByFilter('hltL3fL1DoubleMu10MuOpenOR3p5L1f0L2f10L3Filtered17').empty()"),
   DoubleMu17Mu8_Mu8 = cms.string("!triggerObjectMatchesByFilter('hltL3pfL1DoubleMu10MuOpenL1f0L2pf0L3PreFiltered8').empty() || !triggerObjectMatchesByFilter('hltL3pfL1DoubleMu10MuOpenOR3p5L1f0L2pf0L3PreFiltered8').empty()"), 
