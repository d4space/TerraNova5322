...
//void plotHelpComp(std::string iRVar,std::string iVar,std::string iRCut,std::string iCCut,int nptBins,double xbins_pt,std::string iResbosName,std::string iCName,std::string iXAxis) { 
  ...
  TH1D* h_resbos34;
  f_resbos34 = new TFile("34lResbosWIncl.root");
  h_resbos34 = (TH1D*)f_resbos34->Get("lResbos")->Clone();
  TH1D* h_resbos31;
  f_resbos31 = new TFile("31lResbosWIncl.root");
...
//  lCTree->Draw(lCDraw.str().c_str(),iCCut.c_str());
  ...
  Double_t derr[14] = {0,
    8.42070473632422477e+02, 
    4.57971590512070975e+02,
    2.65172066037934371e+02,
    2.15158508841983888e+02,
    1.47959470188888986e+02,
    1.58727454795494282e+02,
    1.15772508238326751e+02,
    1.40251035379752807e+02,
    9.21194043518147367e+01,
    4.00394161197686245e+01,
    2.22980780635400890e+01,
    1.31093804050125797e+01,
    6.71574928200632471e+00
  };
...
*/  
  ...
  powheg->GetYaxis()->SetRangeUser(0.,2600);
  powheg->SetTitle("");
...
//  gPad->cd();
  ...
  lC0->SaveAs("WInclenPt_Result.png");
...
void theoryStudy_test() {
  ...
  std::string WInclRes = "resbosWIncl30.root";
 ...
 // plotHelpComp("y_B","vy","WT00*(pT_d1 > -25 && pT_d2 > -25 && abs(y_d1) < 12.1 && abs(y_d2) < 12.1)",lRBase,50,-6.,6.,ZRes,lR2,"y Boson ");
  //plotHelpComp("y_B","0.5*log((dilep.e()+dilep.pz())/(dilep.e()-dilep.pz()))","WT00*(pT_d1 > 25 && pT_d2 > 25 && abs(y_d1) < 2.1 && abs(y_d2) < 2.1)","(runNum > 0)*"+lBase,50,-3.,3.,ZRes,Zmumu,"Boson Rapidity");
      ...
      plotHelpComp("pT_B","BornEffCorr","WT00*(pT_d1 > 25 && abs(y_d1) < 2.5 )","(BornEffCorr>0)",600,0,600,WInclRes,WInclenu,"");
//    plotHelpComp("pT_B","BornEffCorr","WT00*(pT_d1 > 25 && abs(y_d1) < 2.5 )","(BornEffCorr>0)",600,0,600,WMRes,WMenu,"");
    //plotHelpComp("pT_B","BornEffCorr","WT00*(pT_d1 > 20 && abs(y_d1) < 2.1 )","(BornEffCorr>0)",600,0,600,WInclRes,WInclmunu,"");
