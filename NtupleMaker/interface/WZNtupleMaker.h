// $Id: WZNtupleMaker.h,v 1.17 2013/09/13 00:09:33 salee Exp $
//
//
...
class WZNtupleMaker : public edm::EDAnalyzer{
 public:
  explicit WZNtupleMaker(const edm::ParameterSet& iConfig);
...
// Variables ==================================================
  ...
  edm::Handle<std::vector<pat::Tau> > tau_hand;
...
WZNtupleMaker::WZNtupleMaker(const edm::ParameterSet& iConfig)
{
...
//    GenZs.Neut_pt	= new std::vector<double>;
    ...
    Ws.Lept1_ptcID	= new std::vector<int>;
...
void WZNtupleMaker::beginRun( const edm::Run& iRun, const edm::EventSetup& iSetup)
{
  //initialization
...
bool WZNtupleMaker::L1TriggerSelection( const edm::Event& iEvent, const edm::EventSetup& iSetup )
{
  // Get L1 Trigger menu
...
void WZNtupleMaker::GetHLTResults(const edm::Event &iEvent, const edm::EventSetup& iSetup)
{
  //Trigger Information----
...
void WZNtupleMaker::endLuminosityBlock(const edm::LuminosityBlock & lumi, const edm::EventSetup & setup)
{
  //cout<<"end lumi "<<endl;
    if(useEventCounter_){
...
bool WZNtupleMaker::checkOverlap(const double & eta, const double & phi, const double & dRval1,const double & reliso1, const double &dRval2, const double & reliso2)
{
  bool overlap = false;
  if( reliso1 < relIso1_ ) {
    overlap = dRval1 < 0.4 ;
...
bool WZNtupleMaker::MatchObjects( const reco::Candidate::LorentzVector& pasObj,
      const reco::Candidate::LorentzVector& proObj,
      bool exact )
{
...
bool WZNtupleMaker::HasDaughter(reco::GenParticleRef genPtcl, int id)
{
...
reco::GenParticleRef WZNtupleMaker::FindDaughter(reco::GenParticleRef mom,int id)
{
...
void WZNtupleMaker::GetGenInfoW(const edm::Event &iEvent, const edm::EventSetup& iSetup)
{
  if (isRD) return;
...
void WZNtupleMaker::GetGenInfoZ(const edm::Event &iEvent, const edm::EventSetup& iSetup)
{
  if(isRD) return;
  GenInfo GenZinfo;
...
void WZNtupleMaker::GetFSRInfoW(const edm::Event &iEvent, const edm::EventSetup& iSetup)
{
  if(isRD) return;
...
double WZNtupleMaker::alphaRatio(double pt){
...
void WZNtupleMaker::LoopMuon(const edm::Event &iEvent, const edm::EventSetup& iSetup)
{
	...
	Lept1_pixelHits=-9999;
      ...
      //if(Lept1_pcIso04_tmp != Lept1_pcIso04) cout<<"WZNtupleMaker: pcIso04 is not the same"<<endl;
...
//	cout<<"relIsoBeta04 : "<<Lept1_relIsoBeta04<<" Event: "<<EventData.EVENT<<endl;
      ...
      NWs++;
      //Fill tree
      Ws.Lept1_ptcID->push_back(GenType::kMuon);
      ...
      for(unsigned j = i+1; j < mu_hand->size(); j++)
      {
	pat::Muon it2 = mu_hand->at(j);
...
void WZNtupleMaker::LoopElectron(const edm::Event &iEvent, const edm::EventSetup& iSetup)
{
	...
	//W recon
      ...
      //Fill W vectors
      NWs++;
      Ws.Lept1_ptcID->push_back(GenType::kElectron);
      Ws.Lept1_genIdxMatch->push_back(idxMatch);
...
//      cout << "W Muon events genMEtTrue : " << genMEtTrue_met->at(0).pt() << endl;
      ...
      for(unsigned j = i+1; j < ele_hand->size(); j++)
      {
	pat::Electron it2 = ele_hand->at(j);
        it2.setP4(it2.pfCandidateRef()->p4());
...
void WZNtupleMaker::LoopTau(const edm::Event &iEvent, const edm::EventSetup& iSetup)
{
    bool goodVtx=false;
    for(unsigned i = 0; i < tau_hand->size(); i++)
    {
      pat::Tau it1 = tau_hand->at(i);
      //it1.setP4(it1.pfCandidateRef()->p4());
      const Ky::Lepton lep1(it1.p4(), (int) it1.charge());
      lepton1->push_back(lep1);
...
//	    if( fabs((*GenWs.PostLept2_id)[iGen]) == GenType::kElectron)
      ...
      NWs++;
      //Fill W vectors
      Ws.Lept1_ptcID->push_back(GenType::kTau);
      Ws.Lept1_genIdxMatch->push_back(idxMatch);
      Ws.Lept1_genDeltaR->push_back(genDeltaR1);
      Ws.Lept1_genDPtRel->push_back(dPtRel1);
...
//      cout << "W Electron events genMEtTrue : " << genMEtTrue_met->at(0).pt() << endl;
      ...
      for(unsigned j = i+1; j < tau_hand->size(); j++)
      {
	pat::Tau it2 = tau_hand->at(j);
        //it2.setP4(it2.pfCandidateRef()->p4());
...
void WZNtupleMaker::endJob()
{
}
void WZNtupleMaker::endRun(edm::Run const&, edm::EventSetup const&)
{
}
void WZNtupleMaker::beginLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
{
}
