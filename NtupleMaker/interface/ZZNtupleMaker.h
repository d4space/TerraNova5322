// $Id: ZZNtupleMaker.h,v 1.17 2013/09/13 00:09:33 salee Exp $
//
//
...
class ZZNtupleMaker : public edm::EDAnalyzer{
 public:
  explicit ZZNtupleMaker(const edm::ParameterSet& iConfig);
...
ZZNtupleMaker::ZZNtupleMaker(const edm::ParameterSet& iConfig)
{
...
void ZZNtupleMaker::beginRun( const edm::Run& iRun, const edm::EventSetup& iSetup)
{
  //initialization
...
bool ZZNtupleMaker::L1TriggerSelection( const edm::Event& iEvent, const edm::EventSetup& iSetup )
{
  // Get L1 Trigger menu
...
void ZZNtupleMaker::GetHLTResults(const edm::Event &iEvent, const edm::EventSetup& iSetup)
{
  //Trigger Information----
...
void ZZNtupleMaker::endLuminosityBlock(const edm::LuminosityBlock & lumi, const edm::EventSetup & setup)
{
  //cout<<"end lumi "<<endl;
    if(useEventCounter_){
...
bool ZZNtupleMaker::checkOverlap(const double & eta, const double & phi, const double & dRval1,const double & reliso1, const double &dRval2, const double & reliso2)
{
  bool overlap = false;
  if( reliso1 < relIso1_ ) {
    overlap = dRval1 < 0.4 ;
...
bool ZZNtupleMaker::MatchObjects( const reco::Candidate::LorentzVector& pasObj,
      const reco::Candidate::LorentzVector& proObj,
      bool exact )
{
...
bool ZZNtupleMaker::HasDaughter(reco::GenParticleRef genPtcl, int id)
{
...
reco::GenParticleRef ZZNtupleMaker::FindDaughter(reco::GenParticleRef mom,int id)
{
...
void ZZNtupleMaker::GetGenInfoZ(const edm::Event &iEvent, const edm::EventSetup& iSetup)
{
  if(isRD) return;
  GenInfo GenZinfo;
...
void ZZNtupleMaker::LoopMuon(const edm::Event &iEvent, const edm::EventSetup& iSetup)
{
	...
	Lept1_pixelHits=-9999;
      ...
      //if(Lept1_pcIso04_tmp != Lept1_pcIso04) cout<<"ZZNtupleMaker: pcIso04 is not the same"<<endl;
...
//	cout<<"relIsoBeta04 : "<<Lept1_relIsoBeta04<<" Event: "<<EventData.EVENT<<endl;
      ...
      for(unsigned j = i+1; j < mu1_hand->size(); j++)
      {
	pat::Muon it2 = mu1_hand->at(j);
...
void ZZNtupleMaker::LoopElectron(const edm::Event &iEvent, const edm::EventSetup& iSetup)
{
    bool goodVtx=false;
...
void ZZNtupleMaker::endRun(edm::Run const&, edm::EventSetup const&)
{
}
void ZZNtupleMaker::beginLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
{
}
