// $Id: MonoJetAnalyzer.h,v 1.17 2013/09/13 00:09:33 salee Exp $
//
//
...
#include "TerraNova/NtupleMaker/interface/MonoJetBranchVars.h"
...
class MonoJetAnalyzer : public edm::EDAnalyzer{
 public:
  explicit MonoJetAnalyzer(const edm::ParameterSet& iConfig);
...
MonoJetAnalyzer::MonoJetAnalyzer(const edm::ParameterSet& iConfig)
{
    ...
    tree = fs->make<TTree>("tree", "Tree for MonoJet");
...
void MonoJetAnalyzer::beginRun(const edm::Run& iRun, const edm::EventSetup& iSetup)
{
  //initialization
...
bool MonoJetAnalyzer::L1TriggerSelection( const edm::Event& iEvent, const edm::EventSetup& iSetup )
{
  // Get L1 Trigger menu
...
void MonoJetAnalyzer::GetHLTResults(const edm::Event &iEvent, const edm::EventSetup& iSetup)
{
  //Trigger Information----
...
bool MonoJetAnalyzer::endLuminosityBlock(edm::LuminosityBlock & lumi, const edm::EventSetup & setup)
{
  //cout<<"end lumi "<<endl;
    if(useEventCounter_){
...
bool MonoJetAnalyzer::checkOverlap(const double & eta, const double & phi, const double & dRval1,const double & reliso1, const double &dRval2, const double & reliso2)
{
  bool overlap = false;
  if( reliso1 < relIso1_ ) {
    overlap = dRval1 < 0.4 ;
...
bool MonoJetAnalyzer::MatchObjects( const reco::Candidate::LorentzVector& pasObj,
      const reco::Candidate::LorentzVector& proObj,
      bool exact )
{
...
void MonoJetAnalyzer::LoopJets(const edm::Event &iEvent, const edm::EventSetup& iSetup)
{
    EvtPass = false;
    bool goodVtx=false;
...
void MonoJetAnalyzer::endRun(edm::Run const&, edm::EventSetup const&)
{
}
void MonoJetAnalyzer::beginLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
{
}
void MonoJetAnalyzer::endLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
{
}
