...
//  TH1F *lHDiff  = new TH1F((lName+"Diff").c_str(),(lName+"Diff").c_str(),nptBins,xbins_pt); lHDiff->Sumw2();
  ...
  lHDiff->GetYaxis()->SetRangeUser(0.4,1.2);
...
//  lCTree->Draw(lCDraw.str().c_str(),iCCut.c_str());
  ...
  Double_t derr[14] = {0,
    2.88958474525319975e+00, 
    2.84810463290940197e+00,
    2.93846898911661114e+00,
    3.01405043089859381e+00,
    3.32843807212932807e+00,
    3.33457643487145150e+00,
    3.85425738631970471e+00,
    4.57800174748765265e+00,
    4.30846840536170106e+00,
    6.84445761181994605e+00,
    1.20720876405036108e+01,
    1.33610553475389793e+01,
    1.67966812198124416e+01};
  ...
  for( int ipt(1);ipt<=nptBins;ipt++)
  {
    ...
    cout<<"ErrorChecking: "<< lData->GetBinError(ipt)<<"\t"<<h1_data->GetBinContent(ipt)*derr[ipt]/100.<<endl;
...
*/  
  ...
  //powheg->GetYaxis()->SetRangeUser(0.,1600);
  powheg->GetYaxis()->SetRangeUser(0.,1800);
  powheg->SetTitle("");
...
void theoryStudy_test() {
  ...
  std::string WPenu = "../RstUnfold/Result_WpToEleNu.root";
  std::string WMenu = "../RstUnfold/Result_WmToEleNu.root";
