...
)
{
  ...
  ***********************/
  ...
  fnamev.push_back("../EventSelection/MuonLowPU/Muon_WpToMuNu_S8_RecoilCorr.root"); typev.push_back(eWpMuNu);
  fnamev.push_back("../EventSelection/MuonLowPU/Muon_WmToMuNu_S8_RecoilCorr.root"); typev.push_back(eWmMuNu);
...
//  double WptBins[18]={0,2.5,5,7.5,10,12.5,15,17.5,20,30,40,50,70,90,150,190,250,600};
  ...
  RooCBShape* QCDCB_m;
  RooRealVar* acbMean;
  RooRealVar* acbMean_p;
  RooRealVar* acbMean_m;
  RooRealVar* acbSigma;
  RooRealVar* acbSigma_p;
  RooRealVar* acbSigma_m;
  RooRealVar* acbA;
  RooRealVar* acbA_p;
  RooRealVar* acbA_m;
  RooRealVar* acbN;
...
//  RaySigma_m	= aqcdm->sigma->getValV();
  RaySigma	= qcd ->sigma->getValV();
  RaySigma_p	= qcdp->sigma->getValV();
  RaySigma_m	= qcdm->sigma->getValV();

//----------------

// Anti-selection PDFs using CB Function
  acbMean    = new RooRealVar("acbMean","acbMean",20,0,METMAX);
  acbMean_p  = new RooRealVar("acbMean_p","acbMean",20,0,METMAX);
  acbMean_m  = new RooRealVar("acbMean_m","acbMean",20,0,METMAX);
  acbSigma   = new RooRealVar("acbSigma","acbSigma",RaySigma,0,METMAX);
  acbSigma_p = new RooRealVar("acbSigma_p","acbSigma",RaySigma_p,0,METMAX);
  acbSigma_m = new RooRealVar("acbSigma_m","acbSigma",RaySigma_m,0,METMAX);
  acbA	     = new RooRealVar("acbA","Gaussian tail alpha",1,-METMAX,METMAX); //tail on right side has negative
  acbA_p     = new RooRealVar("acbA_p","Gaussian tail alpha",1,-METMAX,METMAX); //tail on right side has negative
  acbA_m     = new RooRealVar("acbA_m","Gaussian tail alpha",1,-METMAX,METMAX); //tail on right side has negative
  acbN	     = new RooRealVar("acbN","Normalization",1,0,METMAX);
  acbN_p     = new RooRealVar("acbN_p","Normalization",1,0,METMAX);
  acbN_m     = new RooRealVar("acbN_m","Normalization",1,0,METMAX);
/*  }else{
  acbMean	-> setConstant(kTRUE);
  acbMean_p	-> setConstant(kTRUE);
  acbMean_m	-> setConstant(kTRUE);
  acbSigma	-> setConstant(kTRUE);
  acbSigma_p	-> setConstant(kTRUE);
  acbSigma_m	-> setConstant(kTRUE);
  acbA		-> setConstant(kTRUE);
  acbA_p	-> setConstant(kTRUE);
  acbA_m	-> setConstant(kTRUE);
  acbN		-> setConstant(kTRUE);
  acbN_p	-> setConstant(kTRUE);
  acbN_m	-> setConstant(kTRUE);
  }
*/
  aQCDCB     = new RooCBShape("aQCDCB",  "aQCDCB",pfmet,*acbMean,  *acbSigma,  *acbA,  *acbN);
  aQCDCB_p   = new RooCBShape("aQCDCB_p","aQCDCBp",pfmet,*acbMean_p,*acbSigma_p,*acbA_p,*acbN_p);
  aQCDCB_m   = new RooCBShape("aQCDCB_m","aQCDCBm",pfmet,*acbMean_m,*acbSigma_m,*acbA_m,*acbN_m);

  apdfMet    = new RooAddPdf("apdfMet", "pdfMet", RooArgList(*apdfWm ,*apdfEWK, *aQCDCB  ),RooArgList(*nAntiSig, *nAntiEWK, *nAntiQCD));  
  apdfMetp   = new RooAddPdf("apdfMetp","pdfMetp",RooArgList(*apdfWmp,*apdfEWKp,*aQCDCB_p),RooArgList(*nAntiSigp,*nAntiEWKp,*nAntiQCDp));
  apdfMetm   = new RooAddPdf("apdfMetm","pdfMetm",RooArgList(*apdfWmm,*apdfEWKm,*aQCDCB_m),RooArgList(*nAntiSigm,*nAntiEWKm,*nAntiQCDm));

// QCD Pdfs using CB Function

  QCDCB	    = new RooCBShape("QCDCB",  "QCDCB",pfmet,*acbMean,  *acbSigma,  *acbA,  *acbN);
  QCDCB_p   = new RooCBShape("QCDCB_p","QCDCB_p",pfmet,*acbMean_p,*acbSigma_p,*acbA_p,*acbN_p);
  QCDCB_m   = new RooCBShape("QCDCB_m","QCDCB_m",pfmet,*acbMean_m,*acbSigma_m,*acbA_m,*acbN_m);
  pdfMet    = new RooAddPdf("pdfMet", "pdfMet", RooArgList(*pdfWm ,*pdfEWK, *QCDCB  ),RooArgList(*nSig, *nEWK, *nQCD));  
  pdfMetp   = new RooAddPdf("pdfMetp","pdfMetp",RooArgList(*pdfWmp,*pdfEWKp,*QCDCB_p),RooArgList(*nSigp,*nEWKp,*nQCDp));
  pdfMetm   = new RooAddPdf("pdfMetm","pdfMetm",RooArgList(*pdfWmm,*pdfEWKm,*QCDCB_m),RooArgList(*nSigm,*nEWKm,*nQCDm));
...
//CB function
  pdfMet->plotOn(wmframe,Components(RooArgSet(*pdfEWK,*QCDCB)),FillColor(fillcolorEWK),DrawOption("F"));
  pdfMet->plotOn(wmframe,Components(RooArgSet(*pdfEWK,*QCDCB)),LineColor(linecolorEWK));
  pdfMet->plotOn(wmframe,Components(RooArgSet(*QCDCB)),FillColor(fillcolorQCD),DrawOption("F"));
  pdfMet->plotOn(wmframe,Components(RooArgSet(*QCDCB)),LineColor(linecolorQCD));
...
//CB function
  apdfMet->plotOn(awmframe,Components(RooArgSet(*apdfEWK,*aQCDCB)),FillColor(fillcolorEWK),DrawOption("F"));
  apdfMet->plotOn(awmframe,Components(RooArgSet(*apdfEWK,*aQCDCB)),LineColor(linecolorEWK));
  apdfMet->plotOn(awmframe,Components(RooArgSet(*aQCDCB)),FillColor(fillcolorQCD),DrawOption("F"));
  apdfMet->plotOn(awmframe,Components(RooArgSet(*aQCDCB)),LineColor(linecolorQCD));
...
//CB function
  pdfMetp->plotOn(wmpframe,Components(RooArgSet(*pdfEWKp,*QCDCB_p)),FillColor(fillcolorEWK),DrawOption("F"));
  pdfMetp->plotOn(wmpframe,Components(RooArgSet(*pdfEWKp,*QCDCB_p)),LineColor(linecolorEWK));
  pdfMetp->plotOn(wmpframe,Components(RooArgSet(*QCDCB_p)),FillColor(fillcolorQCD),DrawOption("F"));
  pdfMetp->plotOn(wmpframe,Components(RooArgSet(*QCDCB_p)),LineColor(linecolorQCD));
...
//CB function
  apdfMetp->plotOn(awmpframe,Components(RooArgSet(*apdfEWKp,*aQCDCB_p)),FillColor(fillcolorEWK),DrawOption("F"));
  apdfMetp->plotOn(awmpframe,Components(RooArgSet(*apdfEWKp,*aQCDCB_p)),LineColor(linecolorEWK));
  apdfMetp->plotOn(awmpframe,Components(RooArgSet(*aQCDCB_p)),FillColor(fillcolorQCD),DrawOption("F"));
  apdfMetp->plotOn(awmpframe,Components(RooArgSet(*aQCDCB_p)),LineColor(linecolorQCD));
...
//CB function
  pdfMetm->plotOn(wmmframe,Components(RooArgSet(*pdfEWKm,*QCDCB_m)),FillColor(fillcolorEWK),DrawOption("F"));
  pdfMetm->plotOn(wmmframe,Components(RooArgSet(*pdfEWKm,*QCDCB_m)),LineColor(linecolorEWK));
  pdfMetm->plotOn(wmmframe,Components(RooArgSet(*QCDCB_m)),FillColor(fillcolorQCD),DrawOption("F"));
  pdfMetm->plotOn(wmmframe,Components(RooArgSet(*QCDCB_m)),LineColor(linecolorQCD));
...
//CB function
  apdfMetm->plotOn(awmmframe,Components(RooArgSet(*apdfEWKm,*aQCDCB_m)),FillColor(fillcolorEWK),DrawOption("F"));
  apdfMetm->plotOn(awmmframe,Components(RooArgSet(*apdfEWKm,*aQCDCB_m)),LineColor(linecolorEWK));
  apdfMetm->plotOn(awmmframe,Components(RooArgSet(*aQCDCB_m)),FillColor(fillcolorQCD),DrawOption("F"));
  apdfMetm->plotOn(awmmframe,Components(RooArgSet(*aQCDCB_m)),LineColor(linecolorQCD));
...
//plotAntiMetm->SetYRange(0.1,1500);
  ...
  txtfile << "  Signal: " << nSigp->getVal() << "#pm" << nSigp->getPropagatedError(*fitResp) << endl;
...
//*
  ...
  txtfile << "CB Mean is: " << acbMean->getValV() << endl;
  txtfile << "CB Plus Mean is: " << acbMean_p->getValV() << endl;
  txtfile << "CB Minus Mean is: " << acbMean_m->getValV() << endl;
  txtfile << "CB Sigma is: " << acbSigma->getValV() << endl;
  txtfile << "CB Plus Sigma is: " << acbSigma_p->getValV() << endl;
  txtfile << "CB Minus Sigma is: " << acbSigma_m->getValV() << endl;
  txtfile << "CB alpha is: " << acbA->getValV() << endl;
  txtfile << "CB Plus alpha is: " << acbA_p->getValV() << endl;
  txtfile << "CB Minus alpha is: " << acbA_m->getValV() << endl;
  txtfile << "CB Normalization is: " << acbN->getValV() << endl;
  txtfile << "CB Plus Normalization is: " << acbN_p->getValV() << endl;
  txtfile << "CB Minus Normalization is: " << acbN_m->getValV() << endl;
  txtfile << endl; 
  txtfile.flags(flags);
  txtfile << endl;
  txtfile.close();
...
//  gStyle->SetTextSize(1.1);
  ...
  hs_W_pt->Add(hSigWpt);
  W_ptLgd->AddEntry(hSigWpt,"W#rightarrow#mu#nu","fl");
  ...
  WP_ptLgd->AddEntry(hSigWPpt,"W^{+}#rightarrow#mu#nu","fl");
  ...
  WM_ptLgd->AddEntry(hSigWMpt,"W^{-}#rightarrow#mu#nu","fl");
  ...
  gBenchmark->Show("fitWMuMetCBSimult");
}
