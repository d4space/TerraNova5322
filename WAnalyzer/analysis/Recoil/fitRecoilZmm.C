...
double sigmaFunc(double *x, double *par)
{
  ...
  //return a*x[0]*x[0] + b*x[0] + c; //don't help fcnPFu2sigma2
  return par[0]
    +par[1]*x[0]
    +par[2]*x[0]*x[0];
}
...
//Written on WMuNeu.h=============
  ...
  for(Int_t iBin=0; iBin<nbins; iBin++) {
    sprintf(histName,"hPFu1_%i",iBin);    hPFu1v.push_back(new TH1D(histName,"",100,-100-RecoilBins[iBin],100-RecoilBins[iBin]));    hPFu1v[iBin]->Sumw2();
    sprintf(histName,"hPFu1Bkg_%i",iBin); hPFu1Bkgv.push_back(new TH1D(histName,"",100,-100-RecoilBins[iBin],100-RecoilBins[iBin])); hPFu1Bkgv[iBin]->Sumw2();
    
    sprintf(histName,"hPFu2_%i",iBin);    hPFu2v.push_back(new TH1D(histName,"",100,-100,100));    hPFu2v[iBin]->Sumw2();	       
    sprintf(histName,"hPFu2Bkg_%i",iBin); hPFu2Bkgv.push_back(new TH1D(histName,"",100,-100,100)); hPFu2Bkgv[iBin]->Sumw2();
  }
...
//  plotPFu1sigma1.AddGraph(errBand,"3",kAzure-9,kFullDotSmall);
  ...
  sprintf(fitparam,"p_{0} = %.3f #pm %.3f",fcnPFu1sigma1->GetParameter(0),fcnPFu1sigma1->GetParError(0));
...
//  else           plotPFu1sigma1.SetYRange(0,30);
  ...
  if(pfu1model>=2) {
    ...
    fcnPFu1sigma2->SetParameters(10,0.2,0);
...
//  else           plotPFu2sigma1.SetYRange(0,20);
  ...
  if(pfu2model>=2) {
    ...
    fcnPFu2sigma2->SetParameters(11,0.02,0.0);
...
//    plotPFu2sigma2.AddGraph(errBand,"3",kAzure-9,kFullDotSmall);
    ...
    sprintf(fitparam,"p_{0} = (%.3f #pm %.3f)",fcnPFu2sigma2->GetParameter(0),fcnPFu2sigma2->GetParError(0));
