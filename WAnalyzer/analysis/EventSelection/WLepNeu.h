...
class WLepNeu: public NtupleAna {
public :
  //WMuons	wMuons;
 ...
 */
   ...
   if (WLepNeuTree == 0 ) {
     /****************
      ...
      f->GetObject("WLepNeu/tree",tree);

#else // SINGLE_TREE

      // The following code should be used if you want this class to access a chain
      // of trees.
      TChain * chain = new TChain("WLepNeu/tree","");
      chain->Add("/terranova_1/W_Ntuple/SingleMuRun2012APromptRecoV1Aod/wNtuple_01.root");
      tree = chain;
#endif // SINGLE_TREE

**********************/
     cout<<"Usage: WLepNeu(TTree*... ) "<<endl;
     exit(-1);
   }else{
   ...
   Int_t WLepNeu::GetEntry(Long64_t entry)
   {
   ...
   Long64_t WLepNeu::LoadTree(Long64_t entry)
   {
   ...
   void WLepNeu::Show(Long64_t entry)
   {
...
Double_t WLepNeu::MuonMinusEffiCorrection(double muonPt, double muonEta)
{
  if (muonPt>20. && muonPt <=40.)
  {
...
Int_t WLepNeu::AddMuonCut(int i)
{
  ...
  return 1;
   ...
   Int_t WLepNeu::ElectronCutHighPU(int i)
   {
     //////////////  Ele   V5 =======================================
   ...
   Int_t WLepNeu::ElectronCutSide(int i)
   {
     //RD Scale Correction
     if(Mode == "AllCorrectionsRD")
     {
   ...
   Int_t WLepNeu::ElectronCutSideHighPU(int i)
   {
     double RelComIsoEB = (*W_Lept1_RelisolPtTrks03)[i]+max(0.,((*W_Lept1_RelisoEm03)[i]*(*W_Lept1_pt)[i])-1.)+(*W_Lept1_RelisoHad03)[i];
     double RelComIsoEE = (*W_Lept1_RelisolPtTrks03)[i]+(*W_Lept1_RelisoEm03)[i]+(*W_Lept1_RelisoHad03)[i]; 
     if((*W_Lept1_pt)[i] < 30) return -1;
   ...
   Int_t WLepNeu::AddElectronCut(int i)
   {
   ...
   Int_t WLepNeu::AddElectronCutHighPU(int i)
   {
...
Int_t WLepNeu::TauCutZ(int i)
{
  ...
  return 1;
   ...
   Int_t WLepNeu::ElectronCutZ(int i)
   {
     //////////////  Ele   V5 =======================================
     if( (*Z_Sign)[i] != 0 ) return -1;
   ...
   Int_t WLepNeu::ElectronCutZHighPU(int i)
   {
...
Int_t WLepNeu::DoScaleSmearScan(
    double zlep1Pt,double zlep1Pz,double zlep1En,double zlep1Phi,
    double zlep2Pt,double zlep2Pz,double zlep2En,double zlep2Phi,double TTW)
{
  //TLorentzVector Z_4(
...
Int_t WLepNeu::EtaRange(double lep1Eta,double lep2Eta)
{
  int lep1Range(-1);
  int lep2Range(-1);
  if( AnaChannel=="ElectronLowPU")
