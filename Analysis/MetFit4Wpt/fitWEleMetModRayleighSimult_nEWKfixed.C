...
void fitWEleMetModRayleighSimult_nEWKfixed(const TString  outputDir,   // output directory
...
{
  gBenchmark->Start("fitWEleMetModRayleighSimult_nEWKfixed");
...
//  double WptBins[18]={0,2.5,5,7.5,10,12.5,15,17.5,20,30,40,50,70,90,150,190,250,600};
  ...
  TH1D *hDYMet[NWptBinPlus]; 
  TH1D *hDYMetp[NWptBinPlus]; 
  TH1D *hDYMetm[NWptBinPlus]; 
  ...
  double EWK_TotalEvents,EWKp_TotalEvents,EWKm_TotalEvents;
  double EWK_Err,EWKp_Err,EWKm_Err;
  W_TotalEvents  = 0;
  Wp_TotalEvents = 0;
  Wm_TotalEvents = 0;
  W_Err  = 0;
  ...
  for(UInt_t ifile=0; ifile<fnamev.size(); ifile++) {
	...
	for( int ipt(0); ipt<NWptBinPlus; ipt++)
	{
	  ...
	  sprintf(histName,"hDYMet_%d",ipt);
	  sprintf(histName_org,"h1_W_Neu_pt_%d",ipt);
	  hDYMet[ipt] =(TH1D*)infile->Get(histName_org)->Clone(histName);hDYMet[ipt]->Sumw2();
	  sprintf(histName,"hDYMetm_%d",ipt);
	  sprintf(histName_org,"h1_Wm_Neu_pt_%d",ipt);
	  hDYMetm[ipt]=(TH1D*)infile->Get(histName_org)->Clone(histName);hDYMetm[ipt]->Sumw2();
	  sprintf(histName,"hDYMetp_%d",ipt);
	  sprintf(histName_org,"h1_Wp_Neu_pt_%d",ipt);
	  hDYMetp[ipt]=(TH1D*)infile->Get(histName_org)->Clone(histName);hDYMetp[ipt]->Sumw2();
  ...
  cout<<"DY/Sig"<<endl;
  cout<<hDYMet[0]->Integral()/hWmunuMet[0]->Integral()<<"\t"<<hDYMet[0]->Integral()<<" / "<<hWmunuMet[0]->Integral()<<endl;
  cout<<hDYMetp[0]->Integral()/hWmunuMetp[0]->Integral()<<"\t"<<hDYMetp[0]->Integral()<<" / "<<hWmunuMetp[0]->Integral()<<endl;
  cout<<hDYMetm[0]->Integral()/hWmunuMetm[0]->Integral()<<"\t"<<hDYMetm[0]->Integral()<<" / "<<hWmunuMetm[0]->Integral()<<endl;
  //delete infile;
  //infile=0;// intree=0;   
  //
  ...
  RooRealVar* nEWK[NWptBinPlus];
  RooRealVar* nEWKp[NWptBinPlus];
  RooRealVar* nEWKm[NWptBinPlus];
  
  RooRealVar* nAntiEWK[NWptBinPlus];
  RooRealVar* nAntiEWKp[NWptBinPlus];
  RooRealVar* nAntiEWKm[NWptBinPlus];
...
//Write All Bin Yields Information to txt file
  ...
  for(int ipt(7);ipt<8;ipt++)
  {
    ...
    nEWK[ipt]  = new RooRealVar(histName,histName,hEWKMet[ipt]->Integral());
    sprintf(histName,"nEWKp_%d",ipt);
    nEWKp[ipt]  = new RooRealVar(histName,histName,hEWKMetp[ipt]->Integral());
    sprintf(histName,"nEWKm_%d",ipt);
    nEWKm[ipt]  = new RooRealVar(histName,histName,hEWKMetm[ipt]->Integral());
    
    sprintf(histName,"nAntiEWK_%d",ipt);
    nAntiEWK[ipt]  = new RooRealVar(histName,histName,hAntiEWKMet[ipt]->Integral());
    sprintf(histName,"nAntiEWKp_%d",ipt);
    nAntiEWKp[ipt] = new RooRealVar(histName,histName,hAntiEWKMetp[ipt]->Integral());
    sprintf(histName,"nAntiEWKm_%d",ipt);
    nAntiEWKm[ipt] = new RooRealVar(histName,histName,hAntiEWKMetm[ipt]->Integral());

    if(ipt>9)
    {
      sprintf(histName,"nAntiEWK_%d",ipt);
      nAntiEWK[ipt]  = new RooRealVar(histName,histName,hHighWpTAnti_EWKMet->Integral());
      sprintf(histName,"nAntiEWKp_%d",ipt);
      nAntiEWKp[ipt] = new RooRealVar(histName,histName,hHighWpTAnti_EWKMetp->Integral());
      sprintf(histName,"nAntiEWKm_%d",ipt);
      nAntiEWKm[ipt] = new RooRealVar(histName,histName,hHighWpTAnti_EWKMetm->Integral());
    }
    ...
    //Initial Values
    if(ipt == 0){
      nSig[ipt] -> setVal(74912.8);
      nSigp[ipt]-> setVal(43696.2);
      nSigm[ipt]-> setVal(31216.6);
      nAntiSig[ipt] -> setVal(0.00661013);
      nAntiSigp[ipt]-> setVal(14.9327);
      nAntiSigm[ipt]-> setVal(0.00389024);
      nQCD[ipt] -> setVal(55147.4);
      nQCDp[ipt]-> setVal(28696.5);
      nQCDm[ipt]-> setVal(26450.9);
      nAntiQCD[ipt] -> setVal(41360);
      nAntiQCDp[ipt]-> setVal(21622.5);
      nAntiQCDm[ipt]-> setVal(19737.5);
      qcdmean[ipt] ->setVal(1.12373);
      qcdPmean[ipt]->setVal(0.0637993);
      qcdMmean[ipt] ->setVal(1.12312);
      qcdsigma1[ipt] ->setVal(8.80439);
      qcdPsigma1[ipt]->setVal(10.5448);
      qcdMsigma1[ipt] ->setVal(8.80399);
      qcda1[ipt] ->setVal(0.174477);
      qcdPa1[ipt]->setVal(0.139617);
      qcdMa1[ipt] ->setVal(0.174505);
      aqcdsigma1[ipt] ->setVal(9.25692);
      aqcdPsigma1[ipt]->setVal(11.0215);
      aqcdMsigma1[ipt] ->setVal(9.25612);
    }else if (ipt == 1){
      nSig[ipt] -> setVal(14288.7);
      nSigp[ipt]-> setVal(8183.22);
      nSigm[ipt]-> setVal(5807.91);
      nAntiSig[ipt] -> setVal(53.3603);
      nAntiSigp[ipt]-> setVal(63.1417);
      nAntiSigm[ipt]-> setVal(0.000123684);
      nQCD[ipt] -> setVal(795.895);
      nQCDp[ipt]-> setVal(790.973);
      nQCDm[ipt]-> setVal(303.918);
      nAntiQCD[ipt] -> setVal(463.135);
      nAntiQCDp[ipt]-> setVal(228.505);
      nAntiQCDm[ipt]-> setVal(225.931);
      qcdmean[ipt] ->setVal(17.1101);
      qcdPmean[ipt]->setVal(17.1022);
      qcdMmean[ipt]->setVal(17.0699);
      qcdsigma1[ipt] ->setVal(16.8692);
      qcdPsigma1[ipt]->setVal(10.5063);
      qcdMsigma1[ipt]->setVal(14.5);
      qcda1[ipt] ->setVal(-0.104855);
      qcdPa1[ipt]->setVal(0.0690869);
      qcdMa1[ipt]->setVal(-0.0321277);
      aqcdsigma1[ipt] ->setVal(12.0796);
      aqcdPsigma1[ipt]->setVal(10.5063);
      aqcdMsigma1[ipt]->setVal(11.0866);
    }else if (ipt == 2){
      nSig[ipt] -> setVal(15286.2);
      nSigp[ipt]-> setVal(8939.98);
      nSigm[ipt]-> setVal(6350.4);
      nAntiSig[ipt] -> setVal(67.6969);
      nAntiSigp[ipt]-> setVal(37.6674);
      nAntiSigm[ipt]-> setVal(37.521);
      nQCD[ipt] -> setVal(915.888);
      nQCDp[ipt]-> setVal(467.246);
      nQCDm[ipt]-> setVal(445.404);
      nAntiQCD[ipt] -> setVal(1126.81);
      nAntiQCDp[ipt]-> setVal(538.888);
      nAntiQCDm[ipt]-> setVal(580.379);
      qcdmean[ipt] ->setVal(12.3845);
      qcdPmean[ipt]->setVal(12.3856);
      qcdMmean[ipt]->setVal(12.3909);
      qcdsigma1[ipt] ->setVal(6.8573);
      qcdPsigma1[ipt]->setVal(6.8341);
      qcdMsigma1[ipt]->setVal(7.02999);
      qcda1[ipt] ->setVal(0.142026);
      qcdPa1[ipt]->setVal(0.15318);
      qcdMa1[ipt]->setVal(0.116095);
      aqcdsigma1[ipt] ->setVal(7.60362);
      aqcdPsigma1[ipt]->setVal(7.05681);
      aqcdMsigma1[ipt]->setVal(8.34369);
    }else if (ipt == 3){
      nSig[ipt] -> setVal(12028.7);
      nSigp[ipt]-> setVal(7185.92);
      nSigm[ipt]-> setVal(4842.78);
      nAntiSig[ipt] -> setVal(35.9686);
      nAntiSigp[ipt]-> setVal(25.4764);
      nAntiSigm[ipt]-> setVal(7.84693e-05);
      nQCD[ipt] -> setVal(2445.72);
      nQCDp[ipt]-> setVal(1210.2);
      nQCDm[ipt]-> setVal(1235.52);
      nAntiQCD[ipt] -> setVal(2311.44);
      nAntiQCDp[ipt]-> setVal(1127.5);
      nAntiQCDm[ipt]-> setVal(1183.94);
      qcdmean[ipt] ->setVal(7.38242);
      qcdPmean[ipt]->setVal(7.35149);
      qcdMmean[ipt]->setVal(7.42187);
      qcdsigma1[ipt] ->setVal(7.71841);
      qcdPsigma1[ipt]->setVal(7.4579);
      qcdMsigma1[ipt]->setVal(7.28195);
      qcda1[ipt] ->setVal(0.119073);
      qcdPa1[ipt]->setVal(0.139142);
      qcdMa1[ipt]->setVal(0.152196);
      aqcdsigma1[ipt] ->setVal(7.71841);
      aqcdPsigma1[ipt]->setVal(8.6013);
      aqcdMsigma1[ipt]->setVal(8.10236);
    }else if (ipt == 4){
      nSig[ipt] -> setVal(10719.4);
      nSigp[ipt]-> setVal(6269.32);
      nSigm[ipt]-> setVal(4450.08);
      nAntiSig[ipt] -> setVal(0.0142703);
      nAntiSigp[ipt]-> setVal(4.12314);
      nAntiSigm[ipt]-> setVal(0.0014234);
      nQCD[ipt] -> setVal(6643.7);
      nQCDp[ipt]-> setVal(3388.76);
      nQCDm[ipt]-> setVal(3254.94);
      nAntiQCD[ipt] -> setVal(5465.43);
      nAntiQCDp[ipt]-> setVal(2801.85);
      nAntiQCDm[ipt]-> setVal(2663.58);
      qcdmean[ipt] ->setVal(5.48715);
      qcdPmean[ipt]->setVal(5.5545);
      qcdMmean[ipt]->setVal(5.63327);
      qcdsigma1[ipt] ->setVal(4.47146);
      qcdPsigma1[ipt]->setVal(4.48011);
      qcdMsigma1[ipt]->setVal(5.47);
      qcda1[ipt] ->setVal(0.226424);
      qcdPa1[ipt]->setVal(0.228891);
      qcdMa1[ipt]->setVal(0.219147);
      qcdsigma2[ipt] ->setVal(7.98871);
      qcdPsigma2[ipt]->setVal(7.75145);
      qcdMsigma2[ipt]->setVal(5.41427);
      qcda2[ipt] ->setVal(0.158548);
      qcdPa2[ipt]->setVal(0.168559);
      qcdMa2[ipt]->setVal(0.140874);
      Frac1[ipt] ->setVal(0.6799);
      Frac1P[ipt]->setVal(0.520174);
      Frac1M[ipt]->setVal(0.816003);
      Frac2[ipt] ->setVal(0.301999);
      Frac2P[ipt]->setVal(0.44578);
      Frac2M[ipt]->setVal(0.234763);
      aqcdsigma1[ipt] ->setVal(4.43041);
      aqcdPsigma1[ipt]->setVal(4.45697);
      aqcdMsigma1[ipt]->setVal(4.86305);
      aqcdsigma2[ipt] ->setVal(9.55735);
      aqcdPsigma2[ipt]->setVal(9.25199);
      aqcdMsigma2[ipt]->setVal(9.90402);
    }else if (ipt == 5){
      nSig[ipt] -> setVal(6021.31);
      nSigp[ipt]-> setVal(3552.13);
      nSigm[ipt]-> setVal(2469.18);
      nAntiSig[ipt] -> setVal(6.47922e-11);
      nAntiSigp[ipt]-> setVal(3.15822e-04);
      nAntiSigm[ipt]-> setVal(1.35471e-08);
      nQCD[ipt] -> setVal(9522.05);
      nQCDp[ipt]-> setVal(4856.96);
      nQCDm[ipt]-> setVal(4665.09);
      nAntiQCD[ipt] -> setVal(6704.24);
      nAntiQCDp[ipt]-> setVal(3385.4);
      nAntiQCDm[ipt]-> setVal(3318.84);
      qcdsigma1[ipt] ->setVal(5.49524);
      qcdPsigma1[ipt]->setVal(5.34933);
      qcdMsigma1[ipt]->setVal(5.74008);
      qcda1[ipt] ->setVal(-0.272376);
      qcdPa1[ipt]->setVal(-0.284812);
      qcdMa1[ipt]->setVal(-0.273621);
      dFrac1[ipt] ->setVal(0.476217);
      dFrac1P[ipt]->setVal(0.5);
      dFrac1M[ipt]->setVal(0.437338);
      qcdsigma2[ipt] ->setVal(6.98619);
      qcdPsigma2[ipt]->setVal(6.92809);
      qcdMsigma2[ipt]->setVal(7.07027);
      qcda2[ipt] ->setVal(0.200925);
      qcdPa2[ipt]->setVal(0.206126);
      qcdMa2[ipt]->setVal(0.198368);
      dFrac2[ipt] ->setVal(0.472434);
      dFrac2P[ipt]->setVal(0.5);
      dFrac2M[ipt]->setVal(0.5);
      aqcdsigma1[ipt] ->setVal(1.2565);
      aqcdPsigma1[ipt]->setVal(1.19166);
      aqcdMsigma1[ipt]->setVal(1.25753);
      aqcdsigma2[ipt] ->setVal(6.74084);
      aqcdPsigma2[ipt]->setVal(6.52739);
      aqcdMsigma2[ipt]->setVal(6.82921);
    }else if (ipt == 6){
      nSig[ipt] -> setVal(5911.32);
      nSigp[ipt]-> setVal(3565.38);
      nSigm[ipt]-> setVal(2624.05);
      nAntiSig[ipt] -> setVal(9.63301e-05);
      nAntiSigp[ipt]-> setVal(4.93256e-05);
      nAntiSigm[ipt]-> setVal(5.72129e-05);
      nQCD[ipt] -> setVal(16051.8);
      nQCDp[ipt]-> setVal(8069.2);
      nQCDm[ipt]-> setVal(7700.76);
      nAntiQCD[ipt] -> setVal(11450.8);
      nAntiQCDp[ipt]-> setVal(5993.79);
      nAntiQCDm[ipt]-> setVal(5460.95);
      qcdsigma1[ipt] ->setVal(10.2472);
      qcdPsigma1[ipt]->setVal(10.2613);
      qcdMsigma1[ipt]->setVal(10.2599);
      qcda1[ipt] ->setVal(0.12532);
      qcdPa1[ipt]->setVal(0.12029);
      qcdMa1[ipt]->setVal(0.121829);
      dFrac1[ipt] ->setVal(0.312996);
      dFrac1P[ipt]->setVal(0.312826);
      dFrac1M[ipt]->setVal(0.316478);
      qcdsigma2[ipt] ->setVal(15.6017);
      qcdPsigma2[ipt]->setVal(15.3222);
      qcdMsigma2[ipt]->setVal(15.5951);
      qcda2[ipt] ->setVal(-0.336621);
      qcdPa2[ipt]->setVal(-0.331216);
      qcdMa2[ipt]->setVal(-0.336461);
      dFrac2[ipt] ->setVal(0.499996);
      dFrac2P[ipt]->setVal(0.499993);
      dFrac2M[ipt]->setVal(0.499989);
      aqcdsigma1[ipt] ->setVal(11.0544);
      aqcdPsigma1[ipt]->setVal(11.0206);
      aqcdMsigma1[ipt]->setVal(11.0247);
      aqcdsigma2[ipt] ->setVal(15.441);
      aqcdPsigma2[ipt]->setVal(15.401);
      aqcdMsigma2[ipt]->setVal(15.419);
    }else if (ipt == 7){
    ...
    if (ipt>0){
      ...
      EWK_TotalEvents  += nEWK[ipt]->getVal();
      EWKp_TotalEvents += nEWKp[ipt]->getVal();
      EWKm_TotalEvents += nEWKm[ipt]->getVal();
      EWK_Err  += nEWK[ipt]->getPropagatedError(*fitRes[ipt]);
      EWKp_Err += nEWKp[ipt]->getPropagatedError(*fitResp[ipt]);
      EWKm_Err += nEWKm[ipt]->getPropagatedError(*fitResm[ipt]);
  ...
  allyields <<"Fit: Tot. EWK: "<<EWK_TotalEvents<<" +/- " <<EWK_Err/13<<" +/- "<<sqrt(EWK_TotalEvents)<<"(stat.)"<<endl;
  allyields <<"Cnt: Tot. EWK: "<<hEWKMet[0]->Integral()<<" +/- " <<sqrt(hEWKMet[0]->Integral())<<"(stat.)"<<endl;
  allyieldsp<<"Fit: Tot. EWK: "<<EWKp_TotalEvents<<" +/- "<<EWKp_Err/13<<" +/- "<<sqrt(EWKp_TotalEvents)<<"(stat.)"<<endl;
  allyieldsp<<"Cnt: Tot. EWK: "<<hEWKMetp[0]->Integral()<<" +/- " <<sqrt(hEWKMetp[0]->Integral())<<"(stat.)"<<endl;
  allyieldsm<<"Fit: Tot. EWK: "<<EWKm_TotalEvents<<" +/- "<<EWKm_Err/13<<" +/- "<<sqrt(EWKm_TotalEvents)<<"(stat.)"<<endl;
  allyieldsm<<"Cnt: Tot. EWK: "<<hEWKMetm[0]->Integral()<<" +/- " <<sqrt(hEWKMetm[0]->Integral())<<"(stat.)"<<endl;
...
//W minus pt distribution
  ...
  gBenchmark->Show("fitWEleMetModRayleighSimult_nEWKfixed");
}
