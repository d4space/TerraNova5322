...
class ZllBase: public ZllNT {
public :
  //WMuons	wMuons;

   double	LumiWeight;
     
   ZllBase(TTree *tree=0,TTree *MuonTree=0, double weight=1,
...
ZllBase::ZllBase(TTree *ZllBaseTree,TTree *WLepTree, double lumiweight,
       ...
       double Wcharge, bool runOnMC, int etaRange_)
{
  ...
  //Bins[18]=    600.;
   ...
   if (ZllBaseTree == 0 ) {
     cout<<"Usage: ZllBase(TTree*... ) "<<endl;
     exit(-1);
   }else{
     cout<<"ZllBase.h: initializing the trees"<<endl;
   ...
   Int_t ZllBase::GetEntry(Long64_t entry)
   {
   ...
   Long64_t ZllBase::LoadTree(Long64_t entry)
   {
   ...
   void ZllBase::Show(Long64_t entry)
   {
...
Double_t ZllBase::ElePlusEffiCorrection(double elePt, double eleEtaSC)
{
...
Double_t ZllBase::EleMinusEffiCorrection(double elePt, double eleEtaSC)
{
  if (elePt>25. && elePt <=35.)
  {
...
Double_t ZllBase::MuonPlusEffiCorrection(double muonPt, double muonEta)
{
...
Double_t ZllBase::MuonMinusEffiCorrection(double muonPt, double muonEta)
{
  if (muonPt>20. && muonPt <=40.)
  {
  ...
  }else if (muonPt>55. )
  {
   ...
   Double_t ZllBase::EleScaleRD(double ele_etaSC)
   {
   ...
   Double_t ZllBase::EleSmearMC(double ele_etaSC)
   {
   ...
   Double_t ZllBase::MuonSmearMC(double mu_eta)
   {
...
Int_t ZllBase::MuonCutSide(int i)
{
  if( !(*Z_Lept1_isGlobal)[i])return -1;
  if((*Z_Lept1_pt)[i] < 20) return -1;
  //if((*Z_Lept1_pt)[i] < 25) return -1;
...
Int_t ZllBase::AddMuonCut(int i)
{
  ...
  return 1;
   ...
   Int_t ZllBase::ElectronCut(int i)
   {
     //RD Scale Correction
   ...
   Int_t ZllBase::ElectronCutHighPU(int i)
   {
     //////////////  Ele   V5 =======================================
   ...
   Int_t ZllBase::ElectronCutSide(int i)
   {
     //RD Scale Correction
     if(Mode == "AllCorrectionsRD")
     {
   ...
   Int_t ZllBase::ElectronCutSideHighPU(int i)
   {
     double RelComIsoEB = (*Z_Lept1_RelisolPtTrks03)[i]+max(0.,((*Z_Lept1_RelisoEm03)[i]*(*Z_Lept1_pt)[i])-1.)+(*Z_Lept1_RelisoHad03)[i];
     double RelComIsoEE = (*Z_Lept1_RelisolPtTrks03)[i]+(*Z_Lept1_RelisoEm03)[i]+(*Z_Lept1_RelisoHad03)[i]; 
     if((*Z_Lept1_pt)[i] < 30) return -1;
   ...
   Int_t ZllBase::AddElectronCut(int i)
   {
   ...
   Int_t ZllBase::AddElectronCutHighPU(int i)
   {
...
Int_t ZllBase::TauCutZ(int i)
{
  ...
  return 1;
   ...
   Int_t ZllBase::ElectronCutZ(int i)
   {
     //////////////  Ele   V5 =======================================
     if( (*Z_Sign)[i] != 0 ) return -1;
     if((*Z_Lept1_pt)[i] < 25) return -1;
   ...
   Int_t ZllBase::ElectronCutZHighPU(int i)
   {
...
Int_t ZllBase::DoScaleSmearScan(
    double zlep1Pt,double zlep1Pz,double zlep1En,double zlep1Phi,
    double zlep2Pt,double zlep2Pz,double zlep2En,double zlep2Phi,double TTW)
{
  //TLorentzVector Z_4(
...
Int_t ZllBase::EtaRange(double lep1Eta,double lep2Eta)
{
  int lep1Range(-1);
  int lep2Range(-1);
...
Int_t ZllBase::EtaRange(double lep1Eta)
{
  int lep1Range(-1);
  if( AnaChannel=="ElectronLowPU")
  {
...
//MisChargeStudy
/*Int_t ZllBase::FillMisChargeInfo()
{
...
Int_t ZllBase::FillAcceptInfo()
{
  return 0;
}
Int_t ZllBase::DumpTruthGenInfo(int i)
{
  return 0;
}

Int_t ZllBase::DoRecoilCorr()
{
  return 0;
}
int ZllBase::DumpMETs()
{
  // MET
...
Int_t ZllBase::InitVar4Evt()
{
  // Recoil
  mTTW = 1;
  mVtxVar.nPrim = 0;
  mVtxVar.nGood = 0;
  //cout<<"ZllBase::InitVar4Evt ==========================="<<endl;
  glbMuChi2=0;
