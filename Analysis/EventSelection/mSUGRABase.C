...
int mSUGRABase::InitVar()
{
  cout<<"Initialize variables at mSUGRABase Class ==================="<<endl;
  mResultDir = OutNameBase+"_"+Mode;

  return 0;
}
void mSUGRABase::Loop()
{
  gBenchmark->Start("mSUGRABase");
  gRandom->SetSeed(0);
//
  if (fChain == 0) return;
   //int Ntries = fChain->GetEntriesFast(); this gives 1234567890 kkk
  Ntries = fChain->GetEntries();
  gBenchmark->Show("mSUGRABase");
}
int mSUGRABase::CheckChannel()
{
