...
int MonoJetBase::InitVar()
{
  cout<<"Initialize variables at MonoJetBase Class ==================="<<endl;
  mResultDir = OutNameBase+"_"+Mode;

  return 0;
}
void MonoJetBase::Loop()
{
  gBenchmark->Start("MonoJetBase");
  gRandom->SetSeed(0);
//
  if (fChain == 0) return;
   //int Ntries = fChain->GetEntriesFast(); this gives 1234567890 kkk
  Ntries = fChain->GetEntries();
  gBenchmark->Show("MonoJetBase");
}
int MonoJetBase::CheckChannel()
{
