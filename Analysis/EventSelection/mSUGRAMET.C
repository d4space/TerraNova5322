...
void mSUGRAMET::Loop()
{
  ...
  Debug=false;
  cout<<"==========================================================="<<endl;
  cout<<"mSUGRAMET Analysis with Mode: "<<Mode<<"  AnaChannel: "<<AnaChannel<<endl;
  cout<<"==========================================================="<<endl;
  gBenchmark->Start("mSUGRAMET");
...
//
  ...
  for (int i(0); i<Ntries;i++)
  {
    ...
    //===================
    //if(mSUGRABase::CheckChannel()!=0) exit(-1); no channel

    //============
    //Trigger Cut
    //============
    //if(mSUGRABase::TriggerCut() !=0) continue;
  ...
  cout<<"Passed mSUGRA evts: "<<mNTTevt<<endl;
  //Results======================
  Fill_METprofiles();
...
//  gSystem->mkdir(mResultDir);
  ...
  myFile->Write();
  gBenchmark->Show("mSUGRAMET");
}
int mSUGRAMET::InitVar()
{
  cout<<"Initialize variable at mSUGRAMET class ==========="<<endl;
  evtCnt = 0;
  mNTTevt = 0;
  return 0;
}
int mSUGRAMET::InitVar4Evt()
{
  //cout<<"mSUGRAMET::InitVar4Evt ==========================="<<endl;
  mSUGRABase::InitVar4Evt();
  return 0;
}
int mSUGRAMET::VertexCut()
{
...
double mSUGRAMET::CalcEvtWeight()
{
...
/*
int mSUGRAMET::TTbestSelect()
{
 
  return 0;
}
*/
int mSUGRAMET::InitHistogram()
{
  myFile=new TFile(mResultDir+"/"+OutNameBase+".root","RECREATE");
  h2_pfMET_pxpy  = new TH2D("h2_pfMET_pxpy","pfMET pxpy",100,-1300,1300,100,-1300,1300);
  h2_MVaMET_pxpy  = new TH2D("h2_MVaMET_pxpy","MVaMET pxpy",100,-1300,1300,100,-1300,1300);
  h2_NoPuMET_pxpy  = new TH2D("h2_NoPuMET_pxpy","NoPuMET pxpy",100,-1300,1300,100,-1300,1300);
  for(int i(0);i<PUrangeBin;i++)
  {
  sprintf(histName,"h1_PF_Met_%d",i);
  h1_PF_Met[i]     = new TH1D(histName,"PF MET",100,0.,1300);
  sprintf(histName,"h1_MVA_Met_%d",i);
  h1_MVA_Met[i]    = new TH1D(histName,"MVA MET",100,0.,1300);
  sprintf(histName,"h1_NoPU_Met_%d",i);
  h1_NoPU_Met[i]   = new TH1D(histName,"NoPU MET",100,0.,1300);
...
int mSUGRAMET::Fill_METs()
{
  ...
  //h2_pfMET_pxpy->Draw("CONT3");
  //h2_MVaMET_pxpy->Draw("CONT3");
  //h2_NoPuMET_pxpy->Draw("CONT3");
...
int mSUGRAMET::Fill_METprofiles()
{
