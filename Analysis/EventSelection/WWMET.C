...
void WWMET::Loop()
{
  ...
  Debug=false;
  cout<<"==========================================================="<<endl;
  cout<<"WWMET Analysis with Mode: "<<Mode<<"  AnaChannel: "<<AnaChannel<<endl;
  cout<<"==========================================================="<<endl;
  gBenchmark->Start("WWMET");
...
//
  ...
  for (int i(0); i<Ntries;i++)
  {
    ...
    //===================
    if(WWBase::CheckChannel()!=0) exit(-1);

    //============
    //Trigger Cut
    //============
    //if(WWBase::TriggerCut() !=0) continue;
...
//    ZbestSelect();
  ...
  myFile->Write();
  gBenchmark->Show("WWMET");
}


void WWMET::Nselected4Bin()
{
...
int WWMET::InitVar()
{
  cout<<"Initialize variable at WWMET class ==========="<<endl;
  evtCnt = 0;
  mNWevt = 0;
  mNZevt = 0;
  for(int i(0);i<NwPtBin;i++)
...
int WWMET::InitVar4Evt()
{
  //cout<<"WWMET::InitVar4Evt ==========================="<<endl;
  WWBase::InitVar4Evt();
  return 0;
}
int WWMET::VertexCut()
...
int WWMET::WWbestSelect()
{
  int NgoodW = 0;
  for(int iw(0); iw<W.size; iw++)
  {
    //additional lepton count
    if((*W_Lept1_ptcID)[iw]==GenType::kMuon )if(AddMuonCut(iw)>0) addLepN++;
    if((*W_Lept1_ptcID)[iw]==GenType::kElectron)if(AddElectronCutHighPU(iw)>0) addLepN++;
    if((*W_Lept1_ptcID)[iw]==GenType::kTau)if(TauCut(iw)>0) addLepN++;
  ...
  if(NgoodW >= 2)W.Pass = true;
	
  return 0;
}
int WWMET::ZbestSelect()
{
  diLeptVtxProb = 0;
  double tmpVar(0);
  double ZLep2PtTmp;
