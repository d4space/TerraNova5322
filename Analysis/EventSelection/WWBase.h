...
// MVAnoPUMETana study
 ...
 //  TH1D*        h1_Zmass_QNoSame[lept1eta][lept2eta];
   ...
   WWBase(TTree *tree=0,TTree *WMuonTree=0, double weight=1,
...
WWBase::WWBase(TTree *WWBaseTree,TTree *WLepTree, double lumiweight,
 ...
 */
   ...
   if (WWBaseTree == 0 ) {
     /****************
      ...
      f->GetObject("WWBase/tree",tree);

#else // SINGLE_TREE

      // The following code should be used if you want this class to access a chain
      // of trees.
      TChain * chain = new TChain("WWBase/tree","");
      chain->Add("/terranova_1/W_Ntuple/SingleMuRun2012APromptRecoV1Aod/wNtuple_01.root");
      tree = chain;
#endif // SINGLE_TREE

**********************/
     cout<<"Usage: WWBase(TTree*... ) "<<endl;
     exit(-1);
   }else{
     cout<<"WWBase.h: initializing the trees"<<endl;
   ...
   Int_t WWBase::GetEntry(Long64_t entry)
   {
   ...
   Long64_t WWBase::LoadTree(Long64_t entry)
   {
   ...
   void WWBase::Show(Long64_t entry)
   {
...
Double_t WWBase::ElePlusEffiCorrection(double elePt, double eleEtaSC)
{
...
Double_t WWBase::EleMinusEffiCorrection(double elePt, double eleEtaSC)
{
  if (elePt>25. && elePt <=35.)
  {
...
Double_t WWBase::MuonPlusEffiCorrection(double muonPt, double muonEta)
{
...
Double_t WWBase::MuonMinusEffiCorrection(double muonPt, double muonEta)
{
  if (muonPt>20. && muonPt <=40.)
  {
...
Int_t WWBase::FillEleZmassDaughEta(int etaRange1, int etaRange2)
{
...
Int_t WWBase::FillMuZmassDaughEta(int etaRange1, int etaRange2)
{
  if((etaRange1==0) && (etaRange2==0))
  ...
  if((etaRange1==4) && (etaRange2==4))
   ...
   Double_t WWBase::EleScaleRD(double ele_etaSC)
   {
   ...
   Double_t WWBase::EleSmearMC(double ele_etaSC)
   {
   ...
   Double_t WWBase::MuonSmearMC(double mu_eta)
   {
...
Int_t WWBase::MuonCutSide(int i)
{
  if( !(*W_Lept1_isGlobal)[i])return -1;
  if((*W_Lept1_pt)[i] < 20) return -1;
  //if((*W_Lept1_pt)[i] < 25) return -1;
...
Int_t WWBase::AddMuonCut(int i)
{
  ...
  return 1;
   ...
   Int_t WWBase::ElectronCut(int i)
   {
     //RD Scale Correction
   ...
   Int_t WWBase::ElectronCutHighPU(int i)
   {
     //////////////  Ele   V5 =======================================
   ...
   Int_t WWBase::ElectronCutSide(int i)
   {
     //RD Scale Correction
     if(Mode == "AllCorrectionsRD")
     {
   ...
   Int_t WWBase::ElectronCutSideHighPU(int i)
   {
     double RelComIsoEB = (*W_Lept1_RelisolPtTrks03)[i]+max(0.,((*W_Lept1_RelisoEm03)[i]*(*W_Lept1_pt)[i])-1.)+(*W_Lept1_RelisoHad03)[i];
     double RelComIsoEE = (*W_Lept1_RelisolPtTrks03)[i]+(*W_Lept1_RelisoEm03)[i]+(*W_Lept1_RelisoHad03)[i]; 
     if((*W_Lept1_pt)[i] < 30) return -1;
   ...
   Int_t WWBase::AddElectronCut(int i)
   {
   ...
   Int_t WWBase::AddElectronCutHighPU(int i)
   {
...
Int_t WWBase::TauCutZ(int i)
{
  ...
  return 1;
   ...
   Int_t WWBase::ElectronCutZ(int i)
   {
     //////////////  Ele   V5 =======================================
     if( (*Z_Sign)[i] != 0 ) return -1;
   ...
   Int_t WWBase::ElectronCutZHighPU(int i)
   {
...
Int_t WWBase::EtaRange(double lep1Eta,double lep2Eta)
{
  int lep1Range(-1);
  int lep2Range(-1);
  if( AnaChannel=="ElectronLowPU")
...
Int_t WWBase::EtaRange(double lep1Eta)
{
  int lep1Range(-1);
  if( AnaChannel=="ElectronLowPU")
  {
...
//MisChargeStudy
/*Int_t WWBase::FillMisChargeInfo()
{
...
Int_t WWBase::DumpTruthGenInfo(int i)
{
  //Gen Level Study
  TruthRecoPost = true;
