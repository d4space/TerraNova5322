...
// MVAnoPUMETana study
 ...
 //  TH1D*        h1_Zmass_QNoSame[lept1eta][lept2eta];
   ...
   WZ3LBase(TTree *tree=0,TTree *WMuonTree=0, double weight=1,
...
WZ3LBase::WZ3LBase(TTree *WZ3LBaseTree,TTree *WLepTree, double lumiweight,
       ...
       double Wcharge, bool runOnMC, int etaRange_)
{
  fChain=0;
   ...
   cout<<"WZ3LBase constructor"<<endl;
 ...
 */
   ...
   if (WZ3LBaseTree == 0 ) {
     /****************
      ...
      f->GetObject("WZ3LBase/tree",tree);

#else // SINGLE_TREE

      // The following code should be used if you want this class to access a chain
      // of trees.
      TChain * chain = new TChain("WZ3LBase/tree","");
      chain->Add("/terranova_1/W_Ntuple/SingleMuRun2012APromptRecoV1Aod/wNtuple_01.root");
      tree = chain;
#endif // SINGLE_TREE

**********************/
     cout<<"Usage: WZ3LBase(TTree*... ) "<<endl;
     exit(-1);
   }else{
     cout<<"WZ3LBase.h: initializing the trees"<<endl;
   ...
   Int_t WZ3LBase::GetEntry(Long64_t entry)
   {
   ...
   Long64_t WZ3LBase::LoadTree(Long64_t entry)
   {
   ...
   void WZ3LBase::Show(Long64_t entry)
   {
...
Double_t WZ3LBase::ElePlusEffiCorrection(double elePt, double eleEtaSC)
{
...
Double_t WZ3LBase::EleMinusEffiCorrection(double elePt, double eleEtaSC)
{
  if (elePt>25. && elePt <=35.)
  {
...
Double_t WZ3LBase::MuonPlusEffiCorrection(double muonPt, double muonEta)
{
...
Double_t WZ3LBase::MuonMinusEffiCorrection(double muonPt, double muonEta)
{
  if (muonPt>20. && muonPt <=40.)
  {
...
Int_t WZ3LBase::FillEleZmassDaughEta(int etaRange1, int etaRange2)
{
...
Int_t WZ3LBase::FillMuZmassDaughEta(int etaRange1, int etaRange2)
{
  if((etaRange1==0) && (etaRange2==0))
  ...
  if((etaRange1==4) && (etaRange2==4))
   ...
   Double_t WZ3LBase::EleScaleRD(double ele_etaSC)
   {
   ...
   Double_t WZ3LBase::EleSmearMC(double ele_etaSC)
   {
   ...
   Double_t WZ3LBase::MuonSmearMC(double mu_eta)
   {
...
Int_t WZ3LBase::MuonCutSide(int i)
{
  if( !(*W_Lept1_isGlobal)[i])return -1;
  if((*W_Lept1_pt)[i] < 20) return -1;
  //if((*W_Lept1_pt)[i] < 25) return -1;
...
Int_t WZ3LBase::AddMuonCut(int i)
{
  ...
  return 1;
   ...
   Int_t WZ3LBase::ElectronCut(int i)
   {
     //RD Scale Correction
   ...
   Int_t WZ3LBase::ElectronCutHighPU(int i)
   {
     //////////////  Ele   V5 =======================================
   ...
   Int_t WZ3LBase::ElectronCutSide(int i)
   {
     //RD Scale Correction
     if(Mode == "AllCorrectionsRD")
     {
   ...
   Int_t WZ3LBase::ElectronCutSideHighPU(int i)
   {
     double RelComIsoEB = (*W_Lept1_RelisolPtTrks03)[i]+max(0.,((*W_Lept1_RelisoEm03)[i]*(*W_Lept1_pt)[i])-1.)+(*W_Lept1_RelisoHad03)[i];
     double RelComIsoEE = (*W_Lept1_RelisolPtTrks03)[i]+(*W_Lept1_RelisoEm03)[i]+(*W_Lept1_RelisoHad03)[i]; 
     if((*W_Lept1_pt)[i] < 30) return -1;
   ...
   Int_t WZ3LBase::AddElectronCut(int i)
   {
   ...
   Int_t WZ3LBase::AddElectronCutHighPU(int i)
   {
...
Int_t WZ3LBase::TauCutZ(int i)
{
  ...
  return 1;
   ...
   Int_t WZ3LBase::ElectronCutZ(int i)
   {
     //////////////  Ele   V5 =======================================
     if( (*Z_Sign)[i] != 0 ) return -1;
   ...
   Int_t WZ3LBase::ElectronCutZHighPU(int i)
   {
...
Int_t WZ3LBase::DoScaleSmearScan(
    double zlep1Pt,double zlep1Pz,double zlep1En,double zlep1Phi,
    double zlep2Pt,double zlep2Pz,double zlep2En,double zlep2Phi,double TTW)
{
  //TLorentzVector Z_4(
...
Int_t WZ3LBase::EtaRange(double lep1Eta,double lep2Eta)
{
  int lep1Range(-1);
  int lep2Range(-1);
  if( AnaChannel=="ElectronLowPU")
...
Int_t WZ3LBase::EtaRange(double lep1Eta)
{
  int lep1Range(-1);
  if( AnaChannel=="ElectronLowPU")
  {
...
//MisChargeStudy
/*Int_t WZ3LBase::FillMisChargeInfo()
{
...
Int_t WZ3LBase::DumpTruthGenInfo(int i)
{
  //Gen Level Study
  TruthRecoPost = true;
...
Int_t WZ3LBase::InitVar4Evt()
{
  ...
  mVtxVar.nPrim = 0;
  mVtxVar.nGood = 0;
  //cout<<"WZ3LBase::InitVar4Evt ==========================="<<endl;
  W.size = W_pt->size();
