{
  ...
  for(Int_t i=14;i>-1;i--)
    {
      ...
      h1_WSide_Neu_eta_0->SetLineColor(bgdColor[i]);
      h1_WSide_Neu_eta_0->SetFillColor(bgdColor[i]);
      h1_WSide_Neu_eta_0->SetFillStyle(1001);


      if( ! (i==0 || i>10) ){  
	  for(Int_t iieta=0;iieta<22;iieta++){
	    TString histnm = "h1_WSide_Neu_eta_"+str_etaId[iieta];
	    h_MET = 0;
	...
	hs_WSide_MET->Add(h1_WSide_Neu_eta_0);   
	lgc->AddEntry(h1_WSide_Neu_eta_0,fileNM[i],"F");
    }
  }  
  ...
  lgc->AddEntry(h1_WSide_Neu_eta_0,"2012RealData 19.619/fb", "LP");
  h1_WSide_Neu_eta_0->SetMarkerStyle(20);  h1_WSide_Neu_eta_0->SetMarkerSize(1); //h1_WSide_Neu_pt->SetMaximum(200000);
  h1_WSide_Neu_eta_0->Draw("e");
  hs_WSide_MET->Draw("same");
  h1_WSide_Neu_eta_0->Draw("same e");
  h1_WSide_Neu_eta_0->GetXaxis()->SetTitle("MET");
  h1_WSide_Neu_eta_0->GetYaxis()->SetTitle("# of event");
  lgc->Draw();
  c1->SaveAs("WSide_Neu_eta_0.png");
  ...
  TCanvas *cvs[22];
    for(Int_t iiieta=0;iiieta<22;iiieta++){
      ...
      TFile *f_fileRD = new TFile(filePath[0]); 
      TString histnmRD = "h1_WSide_Neu_eta_"+str_etaId[iiieta];
