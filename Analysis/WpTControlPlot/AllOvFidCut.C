...
int AllOvFidCut(TString BaseName,TString VarName)
{
  ...
  sprintf(tmpName,BaseName+"_lep_pt");
  pltRatio= new CPlot(tmpName,"","Muon p_{T} [GeV/c]","AllCut/FidCut");
  if(VarName=="lep_eta"){
    sprintf(tmpName,BaseName+"_lep_eta");
    pltRatio= new CPlot(tmpName,"","Muon #eta","AllCut/FidCut");
  }
