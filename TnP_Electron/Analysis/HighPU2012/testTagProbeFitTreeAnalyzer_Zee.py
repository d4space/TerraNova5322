...
InputFileName = "/d1/scratch/kimtaehoon/Ele_HighPU/CMSSW_5_3_8/src/KNUPhy/TnP_Electron/Analysis/HighPU2012/Root_file/testNewWrite_Run2012A_13Jul.root"
...
if isMC:
  ...
  InputFileName = "/d1/scratch/kimtaehoon/Ele_HighPU/CMSSW_5_3_8/src/KNUPhy/TnP_Electron/Analysis/HighPU2012/Root_file/testNewWrite_MC.root"
...
probe_sc_et_all = cms.vdouble( 30,35,40,45,50,55,60,65,70,80 )
probe_sc_et_one = cms.vdouble( 20, 30 )
probe_sc_eta_all= cms.vdouble( -2.5,-1.5,-0.5,0.0,0.5,1.5,2.5 )
#probe_sc_eta_all= cms.vdouble( -2.5,-2.0,-1.5,-1.0,-0.5,0.0,0.5,1.0,1.5,2.0,2.5 )
#probe_sc_eta_all= cms.vdouble( -2.5,-1.5,0.0,1.5,2.5 )
probe_sc_eta_one= cms.vdouble( -3.0,3.0 )
...
##         probe_isWP95 = cms.vstring("probe_isWP95", "dummy[pass=1,fail=0]"),
        probe_isWCut = cms.vstring("probe_isWCut", "dummy[pass=1,fail=0]"),
    ),
...
#     "CBExGaussShape::signalResFail(mass, meanF[2.0946e-01, -5., 5.], sigmaF[8.5695e-04, 0., 5.],alphaF[3.8296e-04], nF[6.7489e+00], sigmaF_2[2.5849e+00], fracF[6.5704e-01])",  ### signal resolution for "fail" sample     
    ...
    ##### Crystal-Ball
     "RooCBShape::signalResPass(mass,peakP[0,-3,3],sigmaP[2,0.1,5],alphaP[0.5,0,3],nP[3,1.5,5])",## the signal function goes here
     "RooCBShape::signalResFail(mass,peakF[0,-3,3],sigmaF[2,0.1,5],alphaF[0.5,0,3],nF[3,1.5,5])",## the signal function goes here
...
#    "FCONV::signalFail(mass, signalPhy, signal)",
    ...
    ##### Gauss 
#    "Gaussian::signalPass(mass, meanP[91.2, 89.0, 93.0], sigmaP[2.3, 0.5, 10.0])",
#    "Gaussian::signalFail(mass, meanF[91.2, 89.0, 93.0], sigmaF[2.3, 0.5, 10.0])",
...
##     ),
      ...
      EfficiencyCategoryAndState = cms.vstring("probe_isWCut","pass"),
      ),
    )
)
#if isMC:
...
########
process.WCutToHLT_et_P = process.PfElectronToId_P.clone()
process.WCutToHLT_et_P.InputDirectoryName = cms.string("WCutToHLT")
process.WCutToHLT_et_P.OutputFileName = cms.string(OutputFilePrefix+"WCutToHLT_et_P.root")
process.WCutToHLT_et_P.Categories = cms.PSet(
    mcTrue = cms.vstring("MC true", "dummy[true=1,false=0]"),           
    probe_passingHLT = cms.vstring("probe_passingHLT", "dummy[pass=1,fail=0]"), 
    )
process.WCutToHLT_et_P.Efficiencies = EfficienciesPsetHLT_SC_et_probeP
process.WCutToHLT_et_M = process.WCutToHLT_et_P.clone()
process.WCutToHLT_et_M.Efficiencies = EfficienciesPsetHLT_SC_et_probeM
process.WCutToHLT_et_M.OutputFileName = cms.string(OutputFilePrefix+"WCutToHLT_et_M.root")
process.WCutToHLT_eta_P = process.WCutToHLT_et_P.clone()
process.WCutToHLT_eta_P.Efficiencies = EfficienciesPsetHLT_SC_eta_probeP
process.WCutToHLT_eta_P.OutputFileName = cms.string(OutputFilePrefix+"WCutToHLT_eta_P.root")
process.WCutToHLT_eta_M = process.WCutToHLT_et_P.clone()
process.WCutToHLT_eta_M.Efficiencies = EfficienciesPsetHLT_SC_eta_probeM
process.WCutToHLT_eta_M.OutputFileName = cms.string(OutputFilePrefix+"WCutToHLT_eta_M.root")
########  
...
#  process.WCutToHLT_et_P +
#  process.WCutToHLT_et_M 
#  process.WCutToHLT_eta_P +
#  process.WCutToHLT_eta_M
    )
